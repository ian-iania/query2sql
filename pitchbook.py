...
# ---------------------------------------------------------
# 9) Main Layout and Processing
# ---------------------------------------------------------
st.title("Query your data on Pitchbook")

# Define custom CSS para aumentar a largura
st.markdown("""
    <style>
        .stTextArea textarea {
            width: 250% !important;
    ...
    if st.button("Consultar"):
        if user_question.strip():
            with st.spinner("Processando sua pergunta..."):
                # Limpa qualquer pergunta/resposta anterior
                if "last_query_shown" in st.session_state:
                    del st.session_state["last_query_shown"]
                ...
                st.session_state["last_query_shown"] = True

                # Processamento específico para perguntas sobre employee relation
                if "employee" in user_question.lower() and "relation" in user_question.lower():
                    fixed_sql = """
                    SELECT c.CompanyName, c.Employees, cehr.EmployeeCount, cehr.EmployeeYear 
                    FROM company c 
                    LEFT JOIN company_employee_history_relation cehr ON c.CompanyID = cehr.CompanyID 
                    WHERE LOWER(c.companyAlsoKnownAs) LIKE LOWER('%Contango%')
                    ORDER BY cehr.EmployeeYear DESC
                    """
                else:
                    # Build QueryEngine e executa query normal
                    engine_llama = build_query_engine(st.session_state["chat_history"])
                    response = engine_llama.query(user_question)
                    fixed_sql = post_process_sql(response.metadata.get("sql_query", "(Não disponível)"))
        ...
        # 3) Second LLM call for final text
        ...
        if rows:
            ...
            st.text_area(
                "Dados",
                value="Nenhum dado retornado ou query sem resultado.",
                height=100,
                disabled=True,
                label_visibility="collapsed"
            )
